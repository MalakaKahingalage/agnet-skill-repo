...
# Registry of generated anchors to ensure they are non-empty and unique
ANCHOR_REGISTRY = {}


def get_unique_anchor(title: str) -> str:
    """
    Generate a non-empty, document-unique anchor ID for a heading title.
    Falls back to 'section' when slugify(title) is empty and de-duplicates
    by appending a numeric suffix (-2, -3, ...) for repeated headings.
    """
    base = slugify(title)
    if not base:
        base = "section"

    count = ANCHOR_REGISTRY.get(base, 0) + 1
    ANCHOR_REGISTRY[base] = count
...
def build_toc_from_md(md_content: str) -> str:
    ...
    for line in md_content.splitlines():
        if line.startswith("## "):
            ...
            entries.append((label, title, anchor))
...
def main():
    ...
    # Load logo SVG — look alongside the script first, then beside the input file
    logo_html = ""
    logo_svg_raw = ""
